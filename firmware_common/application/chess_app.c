...
void create_test_moves() {
    debug_moves[0] = create_move(E, TWO, E, FOUR);
    debug_moves[1] = create_move(E, SEVEN, E, FIVE);

    debug_moves[2] = create_move(F, ONE, C, FOUR);
    debug_moves[3] = create_move(B, EIGHT, C, SIX);

    debug_moves[4] = create_move(G, ONE, F, THREE);
    debug_moves[5] = create_move(C, SIX, D, FOUR);

    debug_moves[6] = create_move(E, ONE, G, ONE);
    debug_moves[7] = create_move(D, FOUR, E, TWO);
}
...
void create_game_1_moves() {
    ...
    debug_moves[2] = create_move(G, ONE, F, THREE);
    debug_moves[3] = create_move(B, EIGHT, C, SIX);

    debug_moves[4] = create_move(D, TWO, D, FOUR);
    debug_moves[5] = create_move(E, FIVE, D, FOUR);

    debug_moves[6] = create_move(F, ONE, C, FOUR);
    debug_moves[7] = create_move(F, EIGHT, C, FIVE);

    debug_moves[8] = create_move(C, TWO, C, THREE);
    debug_moves[9] = create_move(D, FOUR, C, THREE);

    debug_moves[10] = create_move(C, FOUR, F, SEVEN);
    debug_moves[11] = create_move(E, EIGHT, F, SEVEN);

    debug_moves[12] = create_move(D, ONE, D, FIVE);
    debug_moves[13] = create_move(F, SEVEN, E, EIGHT);

    debug_moves[14] = create_move(D, FIVE, H, FIVE);
    debug_moves[15] = create_move(G, SEVEN, G, SIX);

    debug_moves[16] = create_move(H, FIVE, C, FIVE);
    debug_moves[17] = create_move(D, SEVEN, D, SIX);

    debug_moves[18] = create_move(C, FIVE, C, THREE);
    debug_moves[19] = create_move(C, EIGHT, G, FOUR);

    debug_moves[20] = create_move(C, THREE, H, EIGHT);
    debug_moves[21] = create_move(G, FOUR, F, THREE);

    debug_moves[22] = create_move(H, EIGHT, G, EIGHT);
    debug_moves[23] = create_move(E, EIGHT, D, SEVEN);

    debug_moves[24] = create_move(G, EIGHT, F, SEVEN);
    debug_moves[25] = create_move(D, EIGHT, E, SEVEN);

    debug_moves[26] = create_move(F, SEVEN, F, THREE);
    debug_moves[27] = create_move(A, EIGHT, F, EIGHT);

    debug_moves[28] = create_move(F, THREE, G, FOUR);
    debug_moves[29] = create_move(D, SEVEN, D, EIGHT);

    debug_moves[30] = create_move(E, ONE, G, ONE);
    debug_moves[31] = create_move(C, SIX, D, FOUR);

    debug_moves[32] = create_move(G, FOUR, D, ONE);
    debug_moves[33] = create_move(E, SEVEN, E, FOUR);

    debug_moves[34] = create_move(C, ONE, G, FIVE);
    debug_moves[35] = create_move(D, EIGHT, C, EIGHT);

    debug_moves[36] = create_move(F, ONE, E, ONE);
    debug_moves[37] = create_move(E, FOUR, F, FIVE);

    debug_moves[38] = create_move(G, FIVE, E, THREE);
    debug_moves[39] = create_move(D, FOUR, C, SIX);

    debug_moves[40] = create_move(B, ONE, C, THREE);
    debug_moves[41] = create_move(C, SIX, E, FIVE);

    debug_moves[42] = create_move(H, TWO, H, THREE);
    debug_moves[43] = create_move(H, SEVEN, H, FIVE);

    debug_moves[44] = create_move(C, THREE, D, FIVE);
    debug_moves[45] = create_move(F, EIGHT, F, SEVEN);

    debug_moves[46] = create_move(E, THREE, F, FOUR);
    debug_moves[47] = create_move(G, SIX, G, FIVE);

    debug_moves[48] = create_move(F, FOUR, E, FIVE);
    debug_moves[49] = create_move(D, SIX, E, FIVE);

    debug_moves[50] = create_move(D, ONE, E, TWO);
    debug_moves[51] = create_move(G, FIVE, G, FOUR);

    debug_moves[52] = create_move(E, TWO, E, FIVE);
    debug_moves[53] = create_move(F, FIVE, F, TWO);

    debug_moves[54] = create_move(G, ONE, H, ONE);
    debug_moves[55] = create_move(B, SEVEN, B, SIX);

    debug_moves[56] = create_move(D, FIVE, E, SEVEN);
    debug_moves[57] = create_move(C, EIGHT, B, EIGHT);

    debug_moves[58] = create_move(E, SEVEN, C, SIX);
    debug_moves[59] = create_move(B, EIGHT, C, EIGHT);

    debug_moves[60] = create_move(A, ONE, D, ONE);
    debug_moves[61] = create_move(G, FOUR, H, THREE);

    debug_moves[62] = create_move(E, FIVE, E, SIX);
    debug_moves[63] = create_move(C, EIGHT, B, SEVEN);

    debug_moves[64] = create_move(C, SIX, D, EIGHT);
    debug_moves[65] = create_move(B, SEVEN, B, EIGHT);

    debug_moves[66] = create_move(E, SIX, E, TWO);
    debug_moves[67] = create_move(H, THREE, G, TWO);

    debug_moves[68] = create_move(H, ONE, H, TWO);
    debug_moves[69] = create_move(G, TWO, G, ONE);

    debug_moves[70] = create_move(H, TWO, H, THREE);
    debug_moves[71] = create_move(G, ONE, G, THREE);
}
...
void generate_input_select_square(uint8_t* index, MovementDirection* current_direction, Square square_1, Square square_2) {
...
void generate_button_input(Move move, uint8_t index) {
    ...
    generate_input_select_square(&index, &direction, highlighted_square, move.start);
    generate_input_select_square(&index, &direction, move.start, move.end);
